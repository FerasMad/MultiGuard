...
def horizontal_flip_dct(pil_image: Image.Image,
                        compute_dual_dct_fn,
                        dct_mean: float,
                        dct_std: float) -> torch.Tensor:
...
@torch.no_grad()
def average_v_imgfor(dct_encoder, t_original: torch.Tensor, t_flipped: torch.Tensor) -> torch.Tensor:
...
@torch.no_grad()
def average_v_semantic(fnd_encoder,
                       fnd_inputs_original: dict[str, torch.Tensor],
                       pil_image: Image.Image,
                       prepare_fnd_fn) -> torch.Tensor:
    flipped = pil_image.transpose(Image.FLIP_LEFT_RIGHT)
    ...
    fnd_flipped_merged["clip_pixels"] = fnd_flipped["clip_pixels"].to(fnd_inputs_original["clip_pixels"].device)
    v1 = fnd_encoder(fnd_inputs_original)
    v2 = fnd_encoder(fnd_flipped_merged)
    return 0.5 * (v1 + v2)


def tta_predict(fnd_encoder, dct_encoder, qwen_encoder, fusion,
                pil_image: Image.Image, text: str, *,
                compute_dual_dct_fn, prepare_fnd_fn,
                dct_mean: float, dct_std: float, device: torch.device):
