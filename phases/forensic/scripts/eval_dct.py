...
def main():
    ...
    p.add_argument("--ckpt", type=Path, required=True,
                   help="Path to forensic_dct_model.pth (or best.pt)")
    p.add_argument("--test-cache", type=Path,
                   default=Path("phases/forensic/data/dct_cache/test"))
    p.add_argument("--dct-stats", type=Path,
                   default=Path("phases/forensic/data/dct_stats.json"))
    p.add_argument("--out-json", type=Path,
                   default=Path("phases/forensic/outputs/eval_dct.json"))
    p.add_argument("--out-table", type=Path,
                   default=Path("phases/forensic/outputs/eval_table.md"))
    p.add_argument("--batch-size", type=int, default=64)
    p.add_argument("--num-workers", type=int, default=2)
    p.add_argument("--only", nargs="+", default=None,
                   help="Subset of generators to eval (default: all 8)")
    args = p.parse_args()
    ...
    for g in gens:
        ...
        if row.get("skipped"):
            ...
            continue
        print(f"  {g:12s} AP={row.get('ap', float('nan')):.4f}  "
