...
def main():
    ...
    p.add_argument("--out-train", type=Path,
    ...
    gens = sorted([d.name for d in args.raw.iterdir()
    ...
    for sub in [args.out_train / "train" / "0_real",
    ...
    _safe_log(f"Per gen: {n_train_actual} train + {n_val} val + {args.test_per_gen} test (per class)")
    ...
    for g in gens:
        ...
        for i, src in enumerate(nat_train):
            _link_or_copy(src, args.out_train / "train" / "0_real" / f"{g}_real_train_{i:05d}{src.suffix.lower()}")
        for i, src in enumerate(ai_train):
            _link_or_copy(src, args.out_train / "train" / "1_fake" / f"{g}_fake_train_{i:05d}{src.suffix.lower()}")
        for i, src in enumerate(nat_val):
            _link_or_copy(src, args.out_train / "val" / "0_real" / f"{g}_real_val_{i:05d}{src.suffix.lower()}")
        for i, src in enumerate(ai_val):
            _link_or_copy(src, args.out_train / "val" / "1_fake" / f"{g}_fake_val_{i:05d}{src.suffix.lower()}")
        ...
        summary["per_generator"][g] = {
            "ai_available": len(ai_imgs), "nature_available": len(nat_imgs),
            "train_fake": len(ai_train), "train_real": len(nat_train),
            "val_fake": len(ai_val), "val_real": len(nat_val),
            "test_fake": len(ai_test), "test_real": len(nat_test),
        }
    ...
    _safe_log(f"Totals: train {summary['totals']['train_real']}/{summary['totals']['train_fake']} (real/fake)")
