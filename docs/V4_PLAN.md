...
| # | Task | Output |
|---|---|---|
| 0.1 | Confirm transfer mechanism for bulk datasets (HF download vs file transfer from an existing copy) | Mechanism chosen |
| 0.2 | Verify new PC has stable ≥10 Mbps internet (or LAN to source machine) | Bandwidth confirmed |
| 0.3 | Confirm ≥150 GB free disk on new PC | Disk check |
| 0.4 | Obtain HF token (READ scope) for MMFakeBench + BERT/CLIP auto-download | Token saved to `~/.cache/huggingface/token` on new PC |

#### Step 1 — Stage raw datasets (Day 1)

| Source | Default method | Size | Time |
|---|---|---|---|
| **DGM4** (all manipulation types) | `huggingface-cli download rshaojimmy/DGM4` (after accepting terms in the HF web UI) or copy from an existing checkout | ~80 GB | 4-9 h |
| **VisualNews** (full image set) | Copy from an existing checkout or fetch via the official VisualNews release | ~50 GB | 3-8 h |
| **GenImage** (all 8 generators) | Official GenImage benchmark Drive at https://github.com/GenImage-Dataset/GenImage | ~10 GB | 1-2 h |
| **Qwen2-7B-Instruct** (HF cache) | `huggingface-cli download Qwen/Qwen2-7B-Instruct` | ~15 GB | ~25 min |
| **blur_jpg_prob0.pth** | CNNDetection release | ~100 MB | <1 min |
| **NewsCLIPpings annotations** | Google Drive link in https://github.com/g-luo/news_clippings | ~200 MB | <5 min |
| **MMFakeBench** | `huggingface-cli download liuxuannan/MMFakeBench --repo-type dataset` | ~3 GB | 5 min |
| **BERT + CLIP** | Auto-download on first use | ~1 GB | minutes |

Operators with a local copy of the bulk datasets on another machine may transfer them directly (faster than re-downloading). Such transfer recipes are environment-specific and not committed to the repo.
